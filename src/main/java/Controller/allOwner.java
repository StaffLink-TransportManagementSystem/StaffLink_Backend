...
public class allOwner extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        List<OwnerModel> owners = ownerModel.viewAllOwners();

        Gson gson = new Gson();
        String centerJson = gson.toJson(owners);

        if(owners.size() != 0){
            resp.setStatus(HttpServletResponse.SC_OK);
            out.write("{\"size\": "+ owners.size() +",\"list\":"+ centerJson+"}");
            System.out.println("View all Owner Accounts");
